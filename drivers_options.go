...
// DefineDeployRepositories build links between repo and deployment.
func (a *Forj) DeclareDeployRepositories() (err error) {
	for repoName, repo := range a.f.DeployForjfile().Repos {
		...
		if deploy, found := a.f.GetADeployment(repo.Deployment); found {
			...
			gotrace.Trace("Repo '%s' attached to '%s' deployment", repoName, deploy.Name())
		}
	}
	return
}

// DefineMissingDeployRepositories will create missing repository in memory with default values.
func (a *Forj) DefineMissingDeployRepositories(ffd *forjfile.DeployForgeYaml, warning bool) (err error) {
	// Define Deployments repositories and
	for deployName, deploy := range a.f.GetDeployments() {
		// Ensure deploy repo is identified.
		repoName := a.w.Organization + "-" + deployName

		if deploy.AttachedRepo() != nil {
			continue
		}

		if r, found := ffd.GetRepo(repoName); found {
			if r.IsInfra() {
